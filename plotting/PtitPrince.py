...
import pandas as pd
import pingouin as pg # for 95% CI
...
class _Half_ViolinPlotter(_CategoricalPlotter):
    ...
    def __init__(self, x, y, hue, data, order, hue_order,
                 ...
                 color, palette, saturation, offset, flip, CI):
    ...
    def draw_violins(self, ax, kws):
        ...
        for i, group_data in enumerate(self.plot_data):
            ...
            kws.update(dict(linewidth=self.linewidth))
                ...
                if self.flip:
                    ...
                    if self.CI:
                        ...
                        fill_func(np.clip(support[ind95], CI95[0], CI95[1]), #supp95,
                                  self.offset + grid[ind95] + density[ind95] * self.dwidth,
                                  self.offset + grid[ind95],
                                  facecolor=tuple([c*0.7 for c in self.colors[i]]),
                                  edgecolor=tuple([c*0.5 for c in self.colors[i]]),
                                  **kws)
                else:
                    fill_func(support,
                              -self.offset + grid - density * self.dwidth,
...
def half_violinplot(x=None, y=None, hue=None, data=None, order=None, hue_order=None,
               ...
               ax=None, offset=.15, flip=False, CI=False, **kwargs):
    ...
    plotter.plot(ax, kwargs)
    return ax, plotter.support, plotter.density
